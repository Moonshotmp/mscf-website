// GET /.netlify/functions/hyrox-resend?key=<HYROX_ADMIN_KEY>&team=<team_id|demo|demo-singles>&which=registrant|partner|team|all[&to=email]
// Resend any HYROX email for a paid team (lost confirmation, partner never got the invite),
// or preview them with team=demo / team=demo-singles (sample data, [TEST]-tagged, sent to `to`).
...
function demoTeam(division = 'doubles') {
  ...
  const t = {
    ...
    heat_id: HEATS.find(h => h.division === division).id, division, test_mode: true, amount_cents: singles ? 39500 : 56400, member_code_valid: true,
    athletes: {
      ...
      partner: singles ? null : { name: 'Sample Partner', email: 'partner@example.com', phone: '(847) 555-0100', member: false, shirt_size: null, addons: { dexa: true }, waiver: null, confirmed_at: null }
    },
    certificates: [], partner_orders: []
  };
  issueCertificates(t, 'registrant', t.athletes.registrant.addons, 'cs_demo');
  if (t.athletes.partner) issueCertificates(t, 'partner', t.athletes.partner.addons, 'cs_demo');
  return t;
}
...
export default async (req) => {
  ...
  let team;
  if (teamId === 'demo' || teamId === 'demo-singles') {
    if (!to) return bad(`team=${teamId} requires &to=<email> so sample emails never go to a real partner address`);
    team = demoTeam(teamId === 'demo-singles' ? 'singles' : 'doubles');
  } else {
  ...
  if (which === 'partner' && !team.athletes.partner) return bad('Singles entry — there is no partner invite to send');
  const sent = {};
  if (which === 'registrant' || which === 'all') sent.registrant = await emailRegistrantConfirmation(team, { to });
  if ((which === 'partner' || which === 'all') && team.athletes.partner) sent.partner = await emailPartnerInvite(team, { to });
  ...
  return json({ ok: true, team_id: team.team_id, sent, note: teamId.startsWith('demo') ? 'Sample data; the partner link inside points at a non-existent team.' : undefined });
};
